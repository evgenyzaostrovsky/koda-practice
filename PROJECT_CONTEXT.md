...
- The former separate pre-run `inspect` Worker request was the measured source of the repeated delay: 2,844 ms of a 2,929 ms trivial Run. Inspection now happens inside the single `run` request. If its AST finds a referenced dataset that is not current, the Worker requests only those bytes and resumes the same request; code without datasets has no preliminary round trip.
...
- Production baseline before this change (warm browser cache): Sandbox ready 6,436 ms. Optimized local verification: initial uncached boot 10,451 ms, cached/restarted boot 4,520 ms; five warm trivial Runs completed in 47–114 ms wall time with 21 ms Python execution. Package and CDN timings vary by network, so the stable target is no repeated runtime/package/file startup on warm Run.
