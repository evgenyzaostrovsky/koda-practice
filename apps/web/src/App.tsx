...
function Layout() {
  ...
  return (
    <div
      className={`app ${focus ? "focus-mode" : ""} ${collapsed ? "sidebar-collapsed" : ""}`}
    >
      {!focus && (
        <>
          <button className="mobile-menu" onClick={() => setMob(!mob)}>
            {mob ? <X /> : <Menu />}
          </button>
...
function DataPreview({ dataset }: { dataset: Record<string, unknown> }) {
...
function Practice() {
  ...
  const nav = useNavigate(),
    qc = useQueryClient(),
    splitRef = useRef<HTMLDivElement>(null);
  const {
    data: e,
    error: exerciseError,
  ...
  const match = eid.match(/(.+)-(\d+)$/),
    ...
    topic = mods.flatMap((m) => m.topics).find((t) => t.slug === slug),
    total = topic?.exercises.length || 3,
    moduleTitle = topic?.title || slug;
  ...
  const [left, setLeft] = useState(
    ...
    [editorH, setEditorH] = useState(
      () => Number(localStorage.getItem("koda:editor-height")) || 430,
    );
  useEffect(() => {
    if (e) {
  ...
  useEffect(() => {
    const f = (x: KeyboardEvent) => {
      if (
        ...
        x.preventDefault();
        x.stopPropagation();
        action.mutate({ submit: x.shiftKey });
      }
    };
    window.addEventListener("keydown", f, true);
    return () => window.removeEventListener("keydown", f, true);
  }, [code, action.isPending]);
  if (exerciseError)
    return (
      <QueryError retry={() => refetchExercise()} pending={isFetching} focus />
    );
  if (!e) return <Loading />;
  const moduleIndex = mods.findIndex((m) => m.slug === slug),
    persist = () => {
      const saved = loadTaskState(e.id);
      saveTaskState(e.id, {
        code,
    ...
    go = (n: number) => {
      ...
      if (n <= total)
        return nav(`/practice/${slug}-${String(n).padStart(3, "0")}`);
      const next = mods[moduleIndex + 1];
      nav(next ? `/practice/${next.topics[0].exercises[0].id}` : "/progress");
    };
  const hint = async () => {
  ...
  return (
    <div className="practice">
      ...
      <div
        ...
        ref={splitRef}
        style={{
          gridTemplateColumns: `minmax(360px,${left}fr) 8px minmax(460px,${100 - left}fr)`,
        }}
      >
        ...
        <section className="solution">
          ...
          <div className="actions">
            <button
              className="secondary"
              onClick={() => action.mutate({ submit: false })}
              disabled={action.isPending}
            >
              {action.isPending ? <Clock3 /> : <Play />} Запустить{" "}
              <kbd>Ctrl/Cmd+Enter</kbd>
            </button>
            <button
              onClick={() => action.mutate({ submit: true })}
              disabled={action.isPending}
            >
              {action.isPending ? <Clock3 /> : <Check />} Проверить{" "}
