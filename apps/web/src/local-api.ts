...
export async function localApi<T>(path:string,init?:RequestInit):Promise<T>{
 ...
 if(path==='/executions/run'||path==='/attempts/submit'){const exercise=exercises.find((item:Json)=>item.id===body.exercise_id);if(!exercise)throw new Error('Задача не найдена');if(path==='/executions/run')return{ok:true,execution_ms:0,result:{kind:'scalar',data:'Код подготовлен. Нажмите «Проверить».'}} as T;const normalize=(code:string)=>code.replace(/\s+/g,' ').trim().replace(/'/g,'"'),passed=normalize(body.code)===normalize(exercise.solution_code);state.attempts=(state.attempts||0)+1;if(passed)state.solved=Array.from(new Set([...(state.solved||[]),exercise.id]));writeProgress(state);return(passed?{ok:true,passed:true,tests_passed:1,tests_total:1,attempt_number:state.attempts,hints_used:(state.hints[exercise.id]||[]).length,xp_earned:exercise.xp,execution_ms:0,approach:exercise.explanation}:{ok:true,passed:false,tests_passed:0,tests_total:1,attempt_number:state.attempts,execution_ms:0,explanation:{what:'Ответ не совпал с эталонным решением.',expected:'Основной метод темы',actual:'Другое выражение',difference:'Проверьте метод, параметры и result.',check:'Сверьтесь с условием.',nudge:exercise.hints[0]}}) as T}
