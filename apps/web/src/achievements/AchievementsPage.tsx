...
export function AchievementsPage() {
  ...
  const visible = families.filter(
    (family) =>
      filter === "all" ||
      (filter === "started" && family.isStarted && !family.isCompleted) ||
      (filter === "not-started" && !family.isStarted) ||
      (filter === "completed" && family.isCompleted),
  );
  const unlocked = Object.keys(model.snapshot.unlocked).length,
    totalXp = Object.values(model.snapshot.unlocked).reduce(
      (total, item) => total + item.xp,
      0,
    );
  const active =
    selected && families.find((family) => family.slug === selected.slug);
  return (
    <section className="ach-page">
      <header className="ach-head">
        ...
        <div className="ach-summary">
          <span>
            <b>
              {unlocked} / {manifest.achievement_count}
            </b>{" "}
            получено
          </span>
          <span>
          ...
          <span>
            <b>{model.stats.currentStreak}</b> серия
          </span>
          <span>
            <b>{model.stats.maxStreak}</b> максимум
          </span>
          <span>
            <Shield />
            {model.stats.stabilizer ? "Стабилизатор" : "Нет стабилизатора"}
          </span>
        </div>
      </header>
      <div className="ach-body">
        <div className="ach-section-head">
          <h2>Линейки достижений</h2>
          <div className="ach-filters" aria-label="Фильтры линеек">
            {[
              ["all", "Все"],
              ["started", "Начатые"],
...
function FamilyPreview({
  ...
  return (
    <button
      className={`family-preview ${family.isStarted ? "started" : "not-started"} ${family.isCompleted ? "completed" : ""}`}
      onClick={onOpen}
      aria-label={
        concealed
          ? "Секретное достижение. Условие скрыто"
          : `${family.name}. ${family.isStarted ? label : "Не начато"}`
      }
    >
      <span className="family-preview-art">
        <img src={`/achievements/${item.def.icon}`} alt="" />
        <i>{family.isStarted ? <Check /> : <Lock />}</i>
      </span>
      <span className="family-preview-copy">
        <b>{concealed ? "Секретное достижение" : family.name}</b>
        <small>
          {family.isStarted
            ? label
            : concealed
      ...
      <em>
        {family.completedCount} / {family.totalCount}
      </em>
    </button>
  );
}
