...
function AuthForm(){
 ...
 return <main className="auth-page"><form className="auth-card" onSubmit={submit}><div className="auth-brand"><span>K</span><div><b>KODA</b><small>Practice</small></div></div><h1>{mode==='login'?'Вход':mode==='signup'?'Создать аккаунт':mode==='forgot'?'Восстановление пароля':'Новый пароль'}</h1>{mode==='signup'&&<label>Имя<input value={name} onChange={e=>setName(e.target.value)} required autoComplete="name"/></label>}{mode!=='recovery'&&<label>Email<input type="email" value={email} onChange={e=>setEmail(e.target.value)} required autoComplete="email"/></label>}{mode!=='forgot'&&<label>Пароль<input type="password" value={password} onChange={e=>setPassword(e.target.value)} minLength={6} required autoComplete={mode==='login'?'current-password':'new-password'}/></label>}{error&&<p className="auth-error">{error}</p>}{message&&<p className="auth-message">{message}</p>}<button disabled={busy}>{busy?'Подождите…':mode==='login'?'Войти':mode==='signup'?'Зарегистрироваться':mode==='forgot'?'Отправить ссылку':'Сохранить пароль'}</button>{mode==='login'&&<button type="button" className="auth-link" onClick={()=>setMode('forgot')}>Забыли пароль?</button>}<p>{mode==='signup'?'Уже есть аккаунт?':'Нет аккаунта?'} <button type="button" className="auth-inline" onClick={()=>setMode(mode==='signup'?'login':'signup')}>{mode==='signup'?'Войти':'Зарегистрироваться'}</button></p></form></main>
}
