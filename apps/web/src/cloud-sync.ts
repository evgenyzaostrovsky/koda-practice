...
export async function loadProfile(){if(!supabase||!user)return null;const{data,error}=await supabase.from('profiles').select('*').single();if(error)throw error;return data}
...
export async function loadAttempts(){if(!supabase||!user)return[];const{data,error}=await supabase.from('solution_attempts').select('*').order('created_at',{ascending:false}).limit(100);if(error)throw error;return data||[]}
